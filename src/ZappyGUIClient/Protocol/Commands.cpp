...
bool MctCommand::execute(WorldState &worldState,
    const std::vector<std::string> &args)
{
    (void)worldState;
    (void)args;
...
bool MctCommand::operator()(WorldState &worldState,
    const std::vector<std::string> &args)
{
    return execute(worldState, args);
}

std::unique_ptr<GUICommand> MctCommand::create()
{
    return std::make_unique<MctCommand>();
}
}
